...
try {
  ...
  assert.equal(JSON.parse(call(['build', '--out', out])).plugins.length, 11);
  ...
  process.stdout.write(`Installed-package smoke passed: agentpatrol@${pkg.version}, ${paths.length} packed files, 4 personas, 7 profiles, 11 plugins.\n`);
} finally {
