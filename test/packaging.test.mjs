...
import { appendFile, link, mkdir, readFile, readdir, rename, symlink, writeFile } from 'node:fs/promises';
...
test('package allowlist contains only active sources, outputs, and documentation', async () => {
  const manifest = JSON.parse(await readFile(path.join(import.meta.dirname, '..', 'package.json'), 'utf8'));
  const catalog = await getCatalog();
  const active = new Set([...catalog.personas, ...catalog.profiles].flatMap(({ skills }) => skills));
  assert.equal(active.size, 38);
  const sourceSkills = await readdir(path.join(import.meta.dirname, '..', 'skills'));
  assert.deepEqual(sourceSkills.sort(), [...active].sort());
  const packagedSkills = manifest.files.filter((entry) => entry.startsWith('skills/')).map((entry) => entry.slice('skills/'.length, -1));
  assert.deepEqual(packagedSkills.sort(), [...active].sort());
  const expected = [
    'bin/', 'src/', 'catalog.json',
    ...catalog.personas.map(({ id }) => `agents/${id}/`),
    'profiles/',
    ...[...active].map((id) => `skills/${id}/`),
    ...catalog.personas.map(({ id }) => `plugins/${id}/`),
    ...catalog.profiles.map(({ id }) => `plugins/profile-${id}/`),
    'schemas/', 'docs/', 'CHANGELOG.md', 'SECURITY.md', 'README.md', 'LICENSE',
  ];
  assert.deepEqual(manifest.files.slice().sort(), expected.sort());
});
...
test('build is self-contained and deterministic, and ignores unrelated sibling output', async (t) => {
  ...
  for (const id of ['unrelated-one', 'unrelated-two']) {
  ...
  assert.equal(result.plugins.length, 11);
  ...
  for (const id of ['unrelated-one', 'unrelated-two']) assert.equal(await readFile(path.join(out, id, 'plugin.json'), 'utf8'), 'user-owned output\n');
