...
- The packages and shared files the Work touches.
- The repository's task graph (`turbo.json`) and workspace roots.

## Output

The scoped commands the Work ran (install, filter, task) plus any
lockfile or graph change with its reason. No other files are produced.

## Rules

- Respect the workspace roots (`apps/*`, `packages/*`): depend across
...
- Keep per-package scripts canonical (`typecheck`, `lint`, `test`,
  `build`, `dev`, `check`); source-only packages get an explicit stub
